...
int main() {
	...
	while(n--){
		...
		if(order == "ls"){
			...
			pair1 = table1.insert(map<string, int>::value_type(filename, timer));
			if(pair1.second){
				table2.insert(map<int, string>::value_type(timer, filename));
			}
		}else if(order == "rm"){
			cin >> com1;
			string filename(com1);
		    ...
		    if(iter != table1.end()){
		    	int timer_tmp = iter->second;
				iter1 = table2.find(timer_tmp);
		    	iter1->second = com2;
				table1.insert(map<string,int>::value_type(com2, timer_tmp));
		    	table1.erase(iter);
			}
		}
