...
const int
        ...
        MAX_STEP = 30,              //最大步数
        //MIN_STEP = 3;             //最小步数
        MAX_VIEITE_VILLAGE_CNT = 5, //最大经过村庄次数    为2 则不允许在工作途中补给(第一关)
...
const std::string palce[10] = {"0.0", "起点", "村庄1", "矿区1", "矿区2", "村庄2", "终点"};
...
void calculate_money(){
    ...
    for (ui i = 0; i < len; ++i)
    {
        if(work_days_array[i] < MIN_WORK_DAY) continue;
        int money = 0;
...
void show_item_by_money() {
    ...
    for (ui index = 0; index < len; ++index)
    {
        ...
        std::cout << std::endl;
        std::cout << ">_ 位置:" << "起点" << " 所需水:" << 0 << " 食物:" << 0 << std::endl;
        for (ui j = 1; j < save_pre_used_infos[i].size(); ++j)
        {
            std::cout
            << ">_ 位置:" << palce[save_pre_used_infos[i][j].first]
...
void dfs(int index, int day_cnt, int cur_pos, int way[],
    ...
    //经过村庄
    if (cur_pos == VILLAGE || cur_pos == VILLAGE2) {
        ++visite_village_cnt;
    ...
    //到达终点
    if (cur_pos == END) {
        if(work_day < MIN_WORK_DAY) return;
        if (is_overweight(pre_info_length, water, food)) return;
        save_pre_(pre_info_length, cur_pos, water, food);
        ...
        // for (int i = MAXN; i > 0; --i)
    {
        ...
        //在矿区  并且下一步也在矿区
        if ((cur_pos == MINE && i == MINE) || (cur_pos == MINE2 && i == MINE2)) {
